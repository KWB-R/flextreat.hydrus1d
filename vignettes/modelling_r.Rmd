...
aggregate_solute <- function(solute, col_aggr = "date")  {
  solute %>% 
  dplyr::mutate(time_diff = c(0, diff(.data$time)),
                ...
                time_c_top = .data$time_diff * .data$c_top,
                time_c_bot = .data$time_diff * .data$c_bot,
                time_integer = as.integer(.data$time)) %>% 
  dplyr::group_by(.data[[col_aggr]]) %>% 
  dplyr::summarise(time_diff = sum(.data$time_diff),
                   c_top = sum(.data$time_c_top)/time_diff,
                   c_bot = sum(.data$time_c_bot)/time_diff) %>% 
...
tmp3 %>% 
  ...
  dplyr::relocate(.data[[col_aggr]]) %>%  
  tidyr::pivot_longer(cols = tidyselect::all_of(tidyselect::all_of(names(tmp3)[c(-1, -2)])),
                      names_to = "parameter", 
                      values_to = "value") %>% 
...
tlevel_aggr_year_hydrologic <- aggregate_tlevel(t_level, col_aggr = "year_hydrologic") %>% 
  dplyr::mutate(year_hydrologic =  as.integer(.data$year_hydrologic)) %>% 
  dplyr::filter(.data$year_hydrologic != 2017) ### filter out as only may-october
```


```{r plot_waterbalance_static}

plot_waterbalance <- function(tlevel_aggr, y_label = "Water Balance Component (mm)") {
